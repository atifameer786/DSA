...
public class j {
    ...
    public static int[] insertionSort(int[] arr) {
        // take one element from left and consider this as sorted element and then pic element from right side one by one
        // put that element at its sorted position in left array 
        // take second element in tmp and put i pointer on tmp and j behing one position j move until -1 i move untile n 
        // check 
        int i;
    ...
    public static void merge(int[] arr, int l, int mid, int h) {
        ...
        // for example if we have two list then find which is smallest add into auxilary list 
    ...
    public static void moveZeroes(int[] nums) {
        ...
        // i start form j+1 because we only need to move zeros so we don't need to check
    ...
    public static int removeDuplicatesOptimal(int[] nums) {
        // In this problem what we are trying to do is keep trak of usinq element in j
        // if we found new uniqe element then first move j by one and then asign new uniqe to jth position
    ...
    public int[] getFloorAndCeilOptimal(int[] nums, int x) {
        ...
        int h = nums.length-1;
        while(lowerBound<h){
            int mid = lowerBound+(h-lowerBound)/2;
            if(nums[mid]<x){
                lowerBound = mid +1;
            }
            else{
        ...
        while (uperBound<h) {
            ...
            int mid = uperBound+(h-uperBound)/2;
            if(nums[mid]<=x){
                uperBound = mid +1;
            }
            else{
        ...
        return new int[] {lowerBound,uperBound};
